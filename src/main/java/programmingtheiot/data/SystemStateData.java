...
public class SystemStateData extends BaseIotData implements Serializable
{
	...
	{
		// Validate system performance data before adding
		if (data != null) {
	...
	{
		// Implement logic to update data as needed
		if (data instanceof SystemStateData) {
			SystemStateData stateData = (SystemStateData) data;
			this.setCommand(stateData.getCommand());
			this.sensorDataList = stateData.getSensorDataList();
			this.sysPerfDataList = stateData.getSystemPerformanceDataList();
		}
	}
